...
/** Draw a circular marker with the given colour at the given position. */
void DrawRaytrace( const Urae::Raytracer *pRT ) {

	Urae::Raytracer::RayPathComponentSet::const_iterator rayIt;
...
void ShowRayPowerHistogram( Urae::Raytracer::TraceReport &tReport ) {
...
void PrintTraceReport( Urae::Raytracer::TraceReport &tReport ) {
...
int main( int argc, char **ppArgv ) {
	...
	try
	{
		...
		std::cout << "Could not initialise URAE. " << e.What() << "\n";
		return -1;
		...
		if ( al_key_down( &k, ALLEGRO_KEY_LCTRL ) ) {
			...
			if ( al_key_down( &k, ALLEGRO_KEY_P ) ) {
				...
				if ( txPlaced ) {
					...
					rt->Execute();
					if ( rxPlaced ) {
						Urae::Raytracer::TraceReport tReport = rt->ComputeK( rxPos, gain );
				...
				std::cout << "Please specify a signal power (current: " << pUrae->GetTransmitPower() << ") ";
				Real Ptx;
				std::cin >> Ptx;
				delete pUrae;
				try
