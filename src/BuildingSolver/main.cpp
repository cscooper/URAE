...
int main( int argc, char **ppArgv ) {
	...
	for ( ; currLink < linkCount; currLink++ ) {
		...
		UraeData::Link *pLink = pUrae->GetSummedLink( currLink );
		int nodeA = pLink->nodeAindex;
		int nodeB = pLink->nodeBindex;
		UraeData::Node *pNodeA = pUrae->GetNode( nodeA );
		UraeData::Node *pNodeB = pUrae->GetNode( nodeB );
	...
	for ( NodeIterator it = nodeLookup.begin(); it != nodeLookup.end(); it++ ) {
		...
		UraeData::Node *currNode = pUrae->GetNode( it->first );
		if ( it->second.size() == 1 ) {
...
// 
// 		Vector2D p = pUrae->GetNode( nvIt->first )->position;
// 
